...
# 🔤 Choose one letter manually
letter = input("Lettre à rechercher (A-Z) : ").strip().upper()
if len(letter) != 1 or not letter.isalpha():
    print("❌ Veuillez entrer une seule lettre de A à Z.")
    exit()
...
try:
    with sync_playwright() as p:
        ...
        while True:
            url = f"https://b2bhint.com/fr/search?q={letter}&type=companies&page={page_number}"
            ...
            try:
                ...
                if is_blocked_by_cloudflare(page):
                    print("    ❌ BLOQUÉ PAR CLOUDFLARE - Arrêt.")
                    break
    ...
    output_file = f"data/entreprises_{letter}.json"
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=4)

    print(f"\n✅ Données sauvegardées dans : {output_file}")
    print(f"📊 Total entreprises collectées pour {letter} : {len(results)}")
